...
### ✅ Рекомендовано: Выделение "горячей" категории в отдельную зону (tag/zone aware sharding)

**Цель:** закрепить все документы категории "Электроника" за выделенным шардом с помощью зон. Это изолирует нагрузку и позволяет независимо масштабировать ресурсы для самой популярной категории независимо от остальных.

Для работы зон необходимо использовать диапазонное шардирование (range sharding), так как хэш-шардирование несовместимо с зонами (зоны оперируют диапазонами значений ключа). Поэтому сначала перейдём на составной диапазонный шард-ключ ]{ category: 1, _id: 1 }], а затем назначим зону.
...
// 4. Добавить зону для категории "electronics"
sh.addShardToZone("shard3", "electronics_zone");   // выделенный шард
...
- **Сохранение точечной маршрутизации** – запросы по `{ category: "electronics", _id: ... }` направляются точно на `shard3`.
...
- Требует ручной настройки и мониторинга. *Смягчение:* автоматизировать через скрипты и CI/CD.

### ❌ Не рекомендовано: Alertmanager + скрипт
...
|------------------------------------|-----------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------|-------------------------------------|------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Заказы (`orders`)                  | Очень высокая (нельзя потерять или дублировать)                       | Объём растёт с числом пользователей (миллионы записей); запись – редкая (единицы в секунду), чтение – редкое | 90% запись (оформление), 10% чтение | Да                     | Требуют транзакционной связки с обновлением остатков товаров. Cassandra не подходит для таких атомарных операций. Оставить в MongoDB.                                                       |
...
**Вывод:** В Cassandra переносим только корзины (с правильной моделью) и сессии. Товары и заказы остаются в MongoDB.

## 10.2. Модель данных в Cassandra

### 1. Корзина (carts и cart_items)
...
ALTER TABLE carts WITH read_repair = 'NONE';

-- Для сессий также отключаем
ALTER TABLE sessions WITH read_repair = 'NONE';
```

Настройка Anti-Entropy Repair через cron (редкая, например раз в месяц):
```bash
0 4 1 * * nodetool repair -pr -keyspace mobile_world -table carts
0 4 1 * * nodetool repair -pr -keyspace mobile_world -table cart_items
```
